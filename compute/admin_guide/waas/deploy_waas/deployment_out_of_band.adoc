...
. (Optional) Enable *VPC traffic mirroring* to allow the mirrored traffic to flow to the Prisma Cloud Defender deployed on another instance.
+
NOTE: Ports cannot be auto-detected when using *VPC traffic mirroring* because the traffic is routed to the Prisma Cloud Defender through the CSP's traffic mirroring service.
+
